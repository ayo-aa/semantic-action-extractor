...
Semantic Action Extractor converts unstructured English text into source-grounded records of verbal and nominal actions. Each record identifies a predicate and the participants or circumstances connected to it without inventing unsupported `actor` or `patient` labels.
...
text
  ...
  -> seven-slot QA-SRL question prediction
  -> role question + grouped answer spans + exact source offsets
```
...
- A **role question** describes that link in QA-SRL form, such as `who approved something?`.
- A **source-grounded span** copies text from the input and records zero-based, end-exclusive Unicode code-point offsets.

The project performs open predicate–argument extraction. It is not a typed event ontology, intent classifier, action-item detector, assignment tracker, or system for determining whether work is planned or complete.
...
- **H1 — grounding:** A source-constrained encoder produces fewer malformed or ungrounded answers than a generative parser while remaining competitive on labeled QA-pair F1.
...
- **H6 — calibrated selection:** Development-set calibration makes predicate and argument confidence useful for selective review without changing extraction accuracy.
...
The versioned `ActionFrame` response remains intentionally compact. It contains:
...
Dataset preparation uses a separate lossless representation. It preserves release and split identity, source and document identifiers, token boundaries, predicate candidates, verbal inflection paradigms, negative nominal-eventivity decisions, all seven raw question slots, question and answer provenance, the grammatical fields supplied by each release, grouped or discontinuous answers, alternative spans, and multiple annotation judgments.

This separation prevents training evidence from being flattened merely to fit a simple inference response. Adapters convert audited dataset records into the research schema; model predictions convert into the public schema only at the serving boundary.
...
A small, frozen challenge set tests constructions resembling support and operations notes that the research corpora may not represent. It contains newly authored or explicitly licensed text. Unless it contains representative real operational text, results are described as **operational-style** performance rather than proof of operational-domain performance.

Annotation guidance, adjudication, exclusions, and a held-out test split are fixed before model development uses the set.

### Data controls

Each adapter records the release, source terms, official split, document identifiers, checksums, preprocessing revision, excluded records, and derived-artifact rights. QA-SRL and QANom source identifiers remain intact because their development and test material overlaps; joint experiments enforce cross-task document-level leakage checks.
...
Candidate-generator recall, predicate-classifier F1, gold-predicate argument F1, and complete-pipeline F1 are reported separately. Verbal eligibility rules explicitly address auxiliaries, light verbs, and stative constructions.
...
The canonical research output preserves QA-SRL role questions. An optional downstream adapter can derive simpler `who`, `what`, `when`, and `where` fields. The adapter receives separate evaluation because this mapping can lose or distort semantic information.
...
- Implement versioned QA-SRL and QANom adapters into the lossless research schema.
...
- Reproduce a T5-small predicate-level parser on the audited QA-SRL and QANom preparation.
...
- Implement raw-text candidate generation.
...
- Compare gold-predicate extraction with the complete pipeline.
...
The primary metric is labeled QA-pair F1 using maximum one-to-one matching, answer-token intersection-over-union of at least `0.5`, and a frozen question-equivalence contract. The scorer name, version, threshold, matching algorithm, predicate source, and equivalence rule accompany every result.

QA-SRL- and QANom-reference modes are also reported for comparison with prior work, even when their thresholds or treatment of missing predicates differ from the project scorer.
...
- gold-predicate and detected-predicate results;
...
Aggregation rejects incomplete seed sets, mixed data revisions, unequal primary-compute allowances, incompatible scorer settings, and result bundles without required provenance.
...
- The research annotation schema preserves the audited supervision without flattening alternatives or judgments.
...
- Both adapters reproduce audited statistics and official splits.
...
- Candidate generation, predicate classification, gold-predicate extraction, and complete-pipeline results remain separate.
