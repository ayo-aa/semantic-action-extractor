...
| Operational-style challenge set | Held-out support and operations constructions | Candidate pilot protocol, guide, rights notice, normalized workbook contract, strict converter, and 20 accepted synthetic notes prepared; human annotation pending | Only newly authored or explicitly licensed text qualifies |
...
- a 20-record pilot workbook with accepted synthetic note text, complete authoring, rights, privacy, and lineage metadata, four empty normalized annotation tables, and no active review entries.
...
The candidate guide defines predicate eligibility, verbal and nominal types, role questions, answer boundaries, mention-qualifier cues, ambiguous cases, and exclusions. Its 20-record pilot is reserved for guide development and is excluded from reported challenge results. The implemented workbook contract separates candidates, qualifier evidence, questions, and answer spans; pins the accepted Authoring fingerprint; checks exact character and derived token spans; and converts one complete annotator/pass into a guide-only evaluation bundle. Any source containing an excluded candidate is quarantined from both reference-side and comparison-side scorer inputs, and a comparison bundle must declare the same source fingerprint and tokenizer version. A human annotator must now complete two passes after a 7–14 day washout, with shuffled order and without viewing the first pass or model output. That comparison can measure only intra-annotator repeatability.

All 20 synthetic notes are accepted for candidate-pilot publication under explicit protocol-level delegation, but no direct note-level review or human annotation pass has occurred. No second human annotator is available, and no independent annotation or disagreement adjudication has occurred. The proposed 50-record development and 150-record test sets have not been created, sealed, or used for model selection. E1 remains incomplete, and no challenge-set performance claim is available. The pilot cannot remove that independent-evaluation gap; it is limited to guide refinement.

Any future scored challenge set requires a separate independent annotation effort: a second human must label every retained development and test record without seeing Ayo’s labels or model predictions; pre-resolution agreement and every disagreement resolution must be recorded; and the final text, labels, exclusions, splits, and fingerprints must be regenerated. Until then, no candidate artifact is gold, adjudicated, or frozen. Unless representative real operational text is available under appropriate terms, reports call the eventual set **operational-style** rather than evidence of operational-domain performance.
