...
The repository currently provides software evidence only: the dependency-free rule baseline runs, and synthetic unit tests exercise the schema, source grounding, predicate-conditioned alignment, BIO handling, model-construction boundary, and generic exact labeled-span scorer. Those checks do not measure extraction accuracy, validate a real BERT runtime, or constitute a public-data reproduction.
...
| `bert-srl-token-type` primitives | Synthetic alignment, BIO, scoring, and injected-model tests | Partially implemented; no adapter, training run, checkpoint, or corpus result |
...
Future quality tables must identify the exact dataset release and rights status, split, annotation view, scorer, role filters, predicate source, seeds, hardware, configuration, and repository revision.
